...
from ultralytics.nn.modules import (
    ...
    v10Detect,
    Zoom_cat,
    Add,ScalSeq,C3k2_MSEARM,GSConv,EfficientVIM_CG_CSP,C3k2_LSBlock,Slim_Efficient_CSP,A2C2f_CG,C3k2_EDMS,HMSF,CSP_MSAS,MSFF,
...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            ...
            if m.i == 11:
                if isinstance(m, Semantic_Information_Decoupling):
                    # 🔥 只在训练阶段读取掩码
                    # 推理阶段 m.masks 会是 None，不会增加额外开销
                    if hasattr(m, 'masks') and m.masks is not None:
                        sid_masks = m.masks  # (mask_fg, mask_bg, mask_uc)
    ...
    def loss(self, batch, preds=None):
        ...
        preds, sid_masks = self.forward(batch["img"]) if preds is None else preds

        # return self.criterion(preds, batch)
        
        # 只在训练模式且使用v8DetectionLossWithMask时才传递掩码
        # 验证和推理阶段不计算掩码损失
        if self.training and isinstance(self.criterion, v8DetectionLossWithMask):
            return self.criterion(preds, batch, sid_masks=sid_masks)
        else:
            # 验证/推理阶段或使用普通损失函数：不传递掩码参数
            return self.criterion(preds, batch)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            def _forward(x):
                ...
                # Handle tuple return (preds, sid_masks) from modified forward method
                if isinstance(result, tuple):
                    result = result[0]
                return result[0] if isinstance(m, (Segment, YOLOESegment, Pose, OBB)) else result
    ...
    def init_criterion(self):
        ...
        # Use v8DetectionLossWithMask if mask loss is enabled, otherwise use v8DetectionLoss
        # Check if mask_loss_weight is set in model args to determine which loss to use
        mask_loss_weight = getattr(self.args, 'mask_loss_weight', None)
        if mask_loss_weight is not None and mask_loss_weight > 0:
            # Get SID loss hyperparameters
            mask_alpha = getattr(self.args, 'mask_alpha', 1.0)
            mask_beta = getattr(self.args, 'mask_beta', 1.0)
            mask_gamma = getattr(self.args, 'mask_gamma', 0.5)
            
            return v8DetectionLossWithMask(
                self, 
...
# # 先导入依赖（文件开头）
# from ultralytics.nn.modules.block import Semantic_Information_Decoupling
#
#
# # 新增 YOLO11WithSIDLoss 类（继承 DetectionModel）
# class YOLO11WithSIDLoss(DetectionModel):
#     def __init__(self, cfg='yolo11n.yaml', ch=3, nc=None, verbose=True):
#         super().__init__(cfg, ch, nc, verbose)
#         self.sid_aux_masks = {}
#
#     def forward(self, x, augment=False, profile=False, visualize=False):
#         self.sid_aux_masks.clear()
#         if augment:
...
#
#             # 提取索引11的SID模块掩码
#             if m.i == 11 and isinstance(m, Semantic_Information_Decoupling):
#                 self.sid_aux_masks = m.aux_masks
#
#             if visualize:
#                 from ultralytics.utils.ops import feature_visualization
#                 feature_visualization(x, m.type, m.i, save_dir=visualize)
#
#         return x, self.sid_aux_masks
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    base_modules = frozenset(
        {
            ...
            SCDown,
            C2fCIB,
            A2C2f,C3k2_MSEARM,GSConv,EfficientVIM_CG_CSP,C3k2_LSBlock,Slim_Efficient_CSP,A2C2f_CG,C3k2_EDMS,CSP_MSAS,
            # Mix, LayerNorm, FCA_Attention, TDP_Attention, GroupGLKA_AttnMap_Simple, TDP_Attention_Wrapper,
            # Semantic_Information_Decoupling, Pred_Layer, ASPP, CrossAttentionBlock, DSAM_CrossAttention, CBR, DSAM_CrossAttention_Wrapper
        }
    )
    repeat_modules = frozenset(  # modules with 'repeat' arguments
        {
            ...
            C2fCIB,
            C2PSA,
            A2C2f,C3k2_MSEARM,EfficientVIM_CG_CSP,C3k2_LSBlock,Slim_Efficient_CSP,A2C2f_CG,C3k2_EDMS,CSP_MSAS
        }
    )
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in (C3k2,EfficientVIM_CG_CSP ) and scale in "mlx":  # for M/L/X sizes
                args[3] = True
        ...
        elif m is TDP_Attention:
            c1 = ch[f[0]]
            c2 = ch[f[1]]
            m_ = TDP_Attention_Wrapper(c1, c2)  # ⬅️ 改成 wrapper
            m_.i, m_.f, m_.type = i, f, str(m)[8:-2].replace("__main__.", "")
            m_.np = sum(x.numel() for x in m_.parameters())
            layers.append(m_)
            ch.append(m_.attn.output_channels)  # 注意这里取原 TDP_Attention 的输出通道数
            save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1)
            if verbose:
                LOGGER.info(f"{i:>3}{str(f):>20}{n_:>3}{m_.np:10.0f}  {m_.type:<45}{[c1, c2]}")
            continue
            ...
            # 获取上一层的通道数
            in_c = ch[f[0]]  # f[0] 是指上一层的输出通道数
            # 创建 DSAM_CrossAttention 模块
            m_ = DSAM_CrossAttention_Wrapper(in_c)  # 传递输入通道数给 DSAM_CrossAttention
            # 为模块设置一些元数据（如索引、类型等）
            m_.i, m_.f, m_.type = i, f, str(m)[8:-2].replace("__main__.", "")
            # 计算模块的参数数量
            ...
            # 将模块添加到 layers 列表中
            layers.append(m_)
            # 更新通道数列表 ch，将 DSAM_CrossAttention 的输出通道数加入 ch
            # 假设 DSAM_CrossAttention 的输出通道数与输入通道数相同（或根据实现修改）
            channel_num = in_c + 32
            ch.append(channel_num)
            # 更新 save 列表
            save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1)
            if verbose:
                LOGGER.info(f"{i:>3}{str(f):>20}{n_:>3}{m_.np:10.0f}  {m_.type:<45}{[in_c]}")
            continue
