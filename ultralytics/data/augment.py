...
class Format:
    ...
    def __call__(self, labels):
        ...
        # 处理掩码：resize到与图像相同的尺寸并转换为tensor
        if "mask_fg" in labels:
            mask_fg = labels.pop("mask_fg")
            if mask_fg is not None:
                # Resize掩码到当前图像尺寸
                if isinstance(mask_fg, np.ndarray):
                    if mask_fg.shape[:2] != (h, w):
                        import cv2
                        mask_fg = cv2.resize(mask_fg, (w, h), interpolation=cv2.INTER_LINEAR)
                    # 确保掩码是 (H, W) 或 (1, H, W)
                    if mask_fg.ndim == 2:
                        mask_fg = mask_fg[None, ...]  # (1, H, W)
                    mask_fg = torch.from_numpy(mask_fg.astype(np.float32))
                labels["mask_fg"] = mask_fg
        
        if "mask_bg" in labels:
            mask_bg = labels.pop("mask_bg")
            if mask_bg is not None:
                if isinstance(mask_bg, np.ndarray):
                    if mask_bg.shape[:2] != (h, w):
                        import cv2
                        mask_bg = cv2.resize(mask_bg, (w, h), interpolation=cv2.INTER_LINEAR)
                    if mask_bg.ndim == 2:
                        mask_bg = mask_bg[None, ...]  # (1, H, W)
                    mask_bg = torch.from_numpy(mask_bg.astype(np.float32))
                labels["mask_bg"] = mask_bg
        
        if "mask_uc" in labels:
            mask_uc = labels.pop("mask_uc")
            if mask_uc is not None:
                if isinstance(mask_uc, np.ndarray):
                    if mask_uc.shape[:2] != (h, w):
                        import cv2
                        mask_uc = cv2.resize(mask_uc, (w, h), interpolation=cv2.INTER_NEAREST)
                    if mask_uc.ndim == 2:
                        mask_uc = mask_uc[None, ...]  # (1, H, W)
                    mask_uc = torch.from_numpy(mask_uc.astype(np.float32))
                labels["mask_uc"] = mask_uc
