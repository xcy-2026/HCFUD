...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        fraction=1.0,
        channels=3,
        mask_dir=None,  # 掩码目录路径（相对于数据集根目录）
    ):
        """
    ...
    def get_image_and_label(self, index):
        ...
        if self.mask_dir is not None:
            mask = self.load_mask(index, label["ori_shape"])
            if mask is not None:
                label["mask_fg"] = mask  # 前景掩码（SAM生成的软伪标签）
                # 背景掩码 = 1 - 前景掩码
                label["mask_bg"] = 1.0 - mask
                # 不确定性掩码：前景和背景差异小的区域
                # 当 mask_fg ≈ 0.5 时，前景和背景概率差异最小，为不确定性区域
                uncertainty_threshold = 0.1
                # |mask_fg - mask_bg| = |mask_fg - (1-mask_fg)| = |2*mask_fg - 1|
                mask_diff = np.abs(2.0 * mask - 1.0)
                label["mask_uc"] = (mask_diff < uncertainty_threshold).astype(np.float32)
        
        return self.update_labels_info(label)
    
    def load_mask(self, index, ori_shape):
        """
        加载掩码文件（.npy格式）
        
        Args:
            index (int): 图像索引
            ori_shape (tuple): 原始图像尺寸 (h, w)
            
        Returns:
            (np.ndarray | None): 掩码数组，形状为 (H, W)，值域 [0, 1]，如果文件不存在则返回None
        """
        ...
        try:
            ...
            img_stem = Path(im_file).stem
            
            # 构建掩码文件路径
            if Path(self.mask_dir).is_absolute():
                mask_path = Path(self.mask_dir) / f"{img_stem}.npy"
            else:
                # 相对路径：相对于数据集根目录
                dataset_root = Path(self.img_path).parent.parent  # 假设结构为 dataset_root/train/images
                mask_path = dataset_root / self.mask_dir / f"{img_stem}.npy"
            
            if mask_path.exists():
                mask = np.load(mask_path)  # 加载掩码（可能是float16或float32）
                # 转换为float32并确保值域在[0, 1]
                mask = mask.astype(np.float32)
                mask = np.clip(mask, 0.0, 1.0)
                # 如果尺寸不匹配，需要resize（但这里先不resize，让数据增强时处理）
                return mask
            else:
                return None
        except Exception as e:
            LOGGER.warning(f"{self.prefix}Failed to load mask for {img_stem}: {e}")
            return None
